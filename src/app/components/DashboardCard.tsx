...
export default function ItemCard({ itemCount, itemType }: ItemCardProps) {
    ...
    const formattedTitle = itemType.charAt(0).toUpperCase() + itemType.slice(1).toLowerCase() + "s"
    ...
    return (
        ...
        >
            <div className="flex items-center">
                ...
                <div>
                    <button onClick={(e) => { e.stopPropagation(); setShowModal(true); }} className="text-blue-600 hover:text-blue-800 transition-colors" title={`Add new ${formattedTitle}`} >
                        <IoAdd size={24} />
                    </button>
                    <AddItemModal
                        showModal={showModal}
                        onOKAction={async (name) => {
                            ...
                            try {
                                const res = await fetch(`/api/new-item?type=${itemType.toUpperCase()}`, { method: "POST", body: new URLSearchParams({ name }), });
                                const data = await res.json();

                                if (res.ok) { setShowModal(false); router.refresh(); } else { alert(data.error || "Failed to add item"); }
                            } catch {
